...
public class BridgeWebView extends WebView implements WebViewJavascriptBridge {
    ...
    private CustomerWebViewClient mCustomterClient;
    ...
    protected BridgeWebViewClient generateBridgeWebViewClient() {
        return new BridgeWebViewClient(this){
            @Override
            protected boolean shouldOverrideUrlLoadingInner(WebView view, String url) {
                if (mCustomterClient != null){
                    mCustomterClient.shouldOverrideUrlLoadingInner(view, url);
                }
                //恒久返回false，返回true的时候网页空白
                return false;
            }
        };
    }

    public void setCustomerWebViewClient(CustomerWebViewClient customterClient){
        this.mCustomterClient = customterClient;
    }
					...
					for (int i = 0; i < list.size(); i++) {
						...
						if (!TextUtils.isEmpty(responseId)) {
                            ...
                            if (!TextUtils.isEmpty(handlerName)) {
                                ...
                                LogUtils.error("JsBridge: java没有找到相应的处理方法"+handlerName);
							}
							if (handler != null){
